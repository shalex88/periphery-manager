#include <iostream>
#include <sstream>
#include "DayCameraProtocol.h"

std::vector<uint8_t> DayCameraProtocol::packData(const std::vector<uint8_t> &data) {
...
void DayCameraProtocol::logPackage(const std::string& message, const DayCameraProtocol::Packet &serialized_packet) const {
    std::cout << message << " " << packetToString(serialized_packet) << std::endl;
}

std::vector<uint8_t> DayCameraProtocol::unpackData(const std::vector<uint8_t> &packet) {
    auto serialized_packet = serialize(packet);

    return serialized_packet.data;
}

uint8_t DayCameraProtocol::calculateChecksum(const std::vector<uint8_t>& data) {
    uint8_t checksum = 0;
...
bool DayCameraProtocol::verifyChecksum(DayCameraProtocol::Packet &packet) {
    return calculateChecksum(packet.data) == packet.checksum;
}

DayCameraProtocol::Packet DayCameraProtocol::serialize(const std::vector<uint8_t> &packet) {
...
std::vector<uint8_t> DayCameraProtocol::deserialize(const Packet &packet) {
...
std::string DayCameraProtocol::packetToString(const DayCameraProtocol::Packet &packet) const {
